...
Material *SceneParser::parseMaterial()
{
    ...
    float shininess = 0, reflective = 0;
    bool is_light = false;
    getToken(token);
    assert(!strcmp(token, "{"));
    while (true)
    {
        ...
        {
            reflective = readFloat();
        }
        else if (strcmp(token, "shininess") == 0)
        {
            shininess = readFloat();
        }
        else if (strcmp(token, "islight") == 0)
        {
            if (readInt() == 1)
                is_light = true;
        }
    ...
    auto *answer = new Material(diffuseColor, shininess, reflective, is_light);
    return answer;
}
